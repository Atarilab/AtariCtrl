...
from atarictrl.controller import Controller, ctrl_registry
from atarictrl.controller.ctrl_cfgs import BeyondMimicCtrlCfg
...
class MotionLoader:
    def __init__(self, motion_file: str, body_indexes: Sequence[int], device: str = "cpu"):
        ...
        self.object_pos_w = data.get("object_pos_w", None)
        self.object_quat_w = data.get("object_quat_w", None)
...
class BeyondMimicCtrl(Controller):
    ...
    @property
    def object_pos_w(self) -> np.ndarray | None:
        if self.motion.object_pos_w is not None:
            object_pos_w_raw = self.motion.object_pos_w[self.timestep].copy()
            object_pos_w = self.motion_init_align.align_pos(object_pos_w_raw)
            return object_pos_w
        else:
            return None

    @property
    def object_quat_w(self) -> np.ndarray | None:
        if self.motion.object_quat_w is not None:
            object_quat_w_raw = self.motion.object_quat_w[self.timestep].copy()[[1, 2, 3, 0]]
            return self.motion_init_align.align_quat(object_quat_w_raw)
        else:
            return None
    ...
    def get_data(self):
        ...
        if self.object_pos_w is not None:
            ctrl_data["object_pos_w"] = self.object_pos_w
        if self.object_quat_w is not None:
            ctrl_data["object_quat_w"] = self.object_quat_w
        return ctrl_data
