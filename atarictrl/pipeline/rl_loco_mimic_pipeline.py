...
from atarictrl.pipeline.pipeline_cfgs import RlLocoMimicPipelineCfg
from atarictrl.pipeline.rl_multi_policy_pipeline import PolicyManager, RlMultiPolicyPipeline
from atarictrl.pipeline.rl_pipeline import PolicyWrapper
from atarictrl.policy import PolicyCfg
...
class PolicyInterpManager(PolicyManager):
    ...
    def switch_to_loco(self):
        ...
        self._interpolate_init(
            ...
            callback_start=lambda: self._set_policy_and_update_env(self.policy_loco_id),
        )
    ...
    def switch_to_mimic(self):
        ...
        self._interpolate_init(
            ...
            callback_end=lambda: self._set_policy_and_update_env(policy_mimic_id),
        )
        
    def _set_policy_and_update_env(self, policy_id):
        super().set_policy(policy_id)
        if hasattr(self.env, "set_control_mode"):
            if policy_id == self.policy_loco_id:
                self.env.set_control_mode("motor")
            else:
                self.env.set_control_mode("position")
