...
class DummyEnv(Environment):
    ...
    def __init__(self, cfg_env: RobotEnvCfg, device="cpu"):
        ...
        elif self.odometry_type == "VICON":
            assert cfg_env.vicon_cfg is not None, "vicon_cfg must be set if odometry_type is 'VICON'"
            from atarictrl.tools.vicon_odometry import ViconOdometry

            self.vicon_odometry = ViconOdometry(cfg_env.vicon_cfg)
...
if __name__ == "__main__":
    # Example usage
    from atarictrl.config.g1.env.g1_dummy_env_cfg import G1DummyEnvCfg
