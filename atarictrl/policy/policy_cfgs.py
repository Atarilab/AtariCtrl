...
from atarictrl.config import ASSETS_DIR, Config
...
class ReachPolicyCfg(PolicyCfg):
    class ObsScalesCfg(Config):
        dof_vel: float = 1.0

    policy_type: str = "ReachPolicy"

    @property
    def policy_file(self) -> str:
        policy_file = ASSETS_DIR / f"models/{self.robot}/reach/policy.pt"
        return policy_file.as_posix()
    ...
    # Commands map for joystick control of hand positions
    # Format: [left_hand_x, left_hand_y, left_hand_z, right_hand_x, right_hand_y, right_hand_z]
    # Each entry is [min, default, max] for remapping joystick axes
    commands_map: list[list[float]] = [
        [-0.3, 0.0, 0.3],  # left_hand_x
        [-0.3, 0.0, 0.3],  # left_hand_y
        [-0.2, 0.0, 0.2],  # left_hand_z
        [-0.3, 0.0, 0.3],  # right_hand_x
        [-0.3, 0.0, 0.3],  # right_hand_y
        [-0.2, 0.0, 0.2],  # right_hand_z
    ]

    # Default hand positions (3D positions for left and right hands)
    default_left_hand_pos: list[float] = [0.0, 0.3, 0.75]
    default_right_hand_pos: list[float] = [0.0, -0.3, 0.75]


class BimanualManipulationPolicyCfg(PolicyCfg):
    class ObsScalesCfg(Config):
        dof_vel: float = 1.0

    policy_type: str = "BimanualManipulationPolicy"

    @property
    def policy_file(self) -> str:
        policy_file = ASSETS_DIR / f"models/{self.robot}/bimanual_contact/policy.pt"
        return policy_file.as_posix()
    ...
    object_dimensions: list[float] = [0.2, 0.2, 0.23]

    # Contact plan configuration
    contact_sequence_length: int = 16  # Length of contact sequence
    contact_duration: float = 1.0  # Duration for each contact sequence in seconds
    num_future_poses: int = 1  # Number of future object goal poses

    # Commands map for joystick control of object goals
    # Format: [object_pos_x, object_pos_y, object_pos_z, object_rot_x, object_rot_y, object_rot_z]
    # Each entry is [min, default, max] for remapping joystick axes [-1, 1] to [min, max]
    # These values represent velocity/rate of change, which will be multiplied by control_dt
    commands_map: list[list[float]] = [
        [-0.5, 0.0, 0.5],  # object_pos_x (from left stick Y) - velocity in m/s
        [-0.5, 0.0, 0.5],  # object_pos_y (from left stick X) - velocity in m/s
        [-0.3, 0.0, 0.3],  # object_pos_z (from triggers) - velocity in m/s
        [-0.5, 0.0, 0.5],  # object_rot_x (from right stick Y) - angular velocity in rad/s
        [-0.5, 0.0, 0.5],  # object_rot_y (from right stick X) - angular velocity in rad/s
        [-0.3, 0.0, 0.3],  # object_rot_z (from additional controls) - angular velocity in rad/s
    ]
    action_beta: float = 0.0
...
class BeyondMimicObjectPolicyCfg(BeyondMimicPolicyCfg):
    policy_type: str = "BeyondMimicObjectPolicy"
    ...
    @property
    def policy_file(self) -> str:
        policy_file = ASSETS_DIR / f"models/{self.robot}/beyondmimic_object/{self.policy_name}.onnx"
        return policy_file.as_posix()
