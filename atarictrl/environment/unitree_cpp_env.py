...
from atarictrl.environment.env_cfgs import UnitreeEnvCfg
from atarictrl.tools.retarget import HandRetarget
...
class UnitreeCppEnv(Environment):
    ...
    def __init__(self, cfg_env: UnitreeEnvCfg, device="cpu"):
        ...
        if self._odometry_type == "ZED":
            ...
            from atarictrl.tools.zed_odometry import ZedOdometry

            self.zed_odometry = ZedOdometry(self.cfg_env.zed_cfg)
        elif self._odometry_type == "VICON":
            assert self.cfg_env.vicon_cfg is not None, "vicon_cfg must be set if odometry_type is 'VICON'"
            from atarictrl.tools.vicon_odometry import ViconOdometry

            self.vicon_odometry = ViconOdometry(self.cfg_env.vicon_cfg)
    ...
    def set_born_place(self, quat: np.ndarray | None = None, pos: np.ndarray | None = None):
        ...
        if self._odometry_type == "ZED":
            ...
            if self.vicon_odometry.is_valid:
                self.vicon_odometry.set_zero()
    ...
    def update(self):
        ...
        if self._odometry_type == "ZED":
            ...
            self.vicon_odometry.update()
            if self.vicon_odometry.is_valid:
                # born place aligned in vicon_odometry
                self._base_quat, self._base_pos = self.vicon_odometry.zero_align.align_transform(self.vicon_odometry.quat, self.vicon_odometry.pos)
                self._base_lin_vel = self.vicon_odometry.lin_vel                
                # self._base_lin_vel = quat_rotate_inverse_np(self.base_quat, self._lin_vel)
                # print("[VICONODOM] - VEL_W: {}".format(self.vicon_odometry..copy()))
                # print("[VICONODOM] - VEL_B: {}".format(self._base_lin_vel.copy()))
        elif self._odometry_type == "DUMMY":
    ...
    def get_data(self):
        """Get environment data, including box states from vicon_odometry if available."""
        env_data = super().get_data()
        
        # Add box states from vicon_odometry if using VICON odometry
        if self._odometry_type == "VICON" and hasattr(self, "vicon_odometry"):
            # Get all segments data from vicon_odometry
            all_segments_data = self.vicon_odometry.get_all_segments_data()
            
            # Look for box segment (case-insensitive)
            box_segment_name = None
            for segment_name in self.vicon_odometry._segment_lookup.values():
                if "box" in segment_name.lower():
                    box_segment_name = segment_name
                    break
            
            # Extract box data if found
            if box_segment_name is not None and self.vicon_odometry.is_segment_valid(box_segment_name):
                # Get box data using the segment name format from get_all_segments_data
                box_pos_key = f"{box_segment_name}/base_pos_w"
                box_quat_key = f"{box_segment_name}/base_quat"
                box_lin_vel_key = f"{box_segment_name}/base_lin_vel_w"
                box_ang_vel_key = f"{box_segment_name}/base_ang_vel_w"
                
                if box_pos_key in all_segments_data:
                    env_data["box/base_quat"], env_data["box/base_pos_w"] = self.vicon_odometry.zero_align.align_transform(all_segments_data[box_quat_key], 
                                                                                                                           all_segments_data[box_pos_key])
                if box_lin_vel_key in all_segments_data:
                    env_data["box/base_lin_vel_w"] = all_segments_data[box_lin_vel_key]
                if box_ang_vel_key in all_segments_data:
                    env_data["box/base_ang_vel_w"] = all_segments_data[box_ang_vel_key]
        
        return env_data


if __name__ == "__main__":
    from atarictrl.config.g1.env.g1_real_env_cfg import G1RealEnvCfg
    ...
    while 1:
        ...
        time.sleep(0.1)
    # print("Exit")
    # from atarictrl.controller import UnitreeCtrl
